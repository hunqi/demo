...
/**
 * This frame has two buttons to fill a combo box from a separate a thread. The "Good" button
...
class SwingThreadFrame extends JFrame {
    public SwingThreadFrame() {
        ...
        goodButton.addActionListener((e) -> new Thread(new GoodWorkerRunnable(comboBox)));
        panel.add(goodButton);

        JButton badButton = new JButton("Bad");
        badButton.addActionListener((e) -> new Thread(new BadWorkerRunnable(comboBox)));
        panel.add(badButton);
 ...
 * in errors because the combo box methods are not synchronized and both the worker thread and
    ...
    public void run() {
        try {
            while (true) {
                ...
                else if (comboBox.getItemCount() > 0) comboBox.removeItemAt(i % comboBox.getItemCount());
                Thread.sleep(1);
            }
